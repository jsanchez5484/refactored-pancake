...
Start with 3 services max: API, Worker, Search. Split when deploy cadence or team boundaries force it.`
  ...
  async function typeResponse(thinkingEl, html) {
    ...
    const plain = html.replace(/<pre><code>[\s\S]*?<\/code><\/pre>/g, (m) => m);
    const parts = [];
    let last = 0;
    const preRegex = /<pre><code>[\s\S]*?<\/code><\/pre>/g;
    let match;
    while ((match = preRegex.exec(html)) !== null) {
      if (match.index > last) parts.push({ type: 'text', content: html.slice(last, match.index) });
      parts.push({ type: 'block', content: match[0] });
    ...
    for (const part of parts) {
      if (part.type === 'block') {
        textEl.insertAdjacentHTML('beforeend', part.content);
        scrollChat();
        await sleep(120);
      } else {
        const chars = part.content.split('');
        for (let i = 0; i < chars.length; i++) {
          textEl.insertAdjacentHTML('beforeend', chars[i]);
          scrollChat();
          const delay = chars[i] === '\n' ? 8 : (Math.random() * 12 + 4);
          await sleep(delay);
        }
